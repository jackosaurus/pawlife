...
**Cross-references:** `docs/bemy-v1-release-plan.md` §8 ("analytics? observability?"), `CLAUDE.md` ("Things to Avoid" — *to be amended* once this plan ships).
...
Apple has required in-app account deletion since June 2022 for any app with sign-up. Bemy has Supabase Auth → this is a hard blocker. **Surface this to `docs/bemy-v1-release-plan.md` as a new item.** Implementation:
...
Slots into the existing v1 ordering in `docs/bemy-v1-release-plan.md` "Suggested release ordering". Currently item 4 is "Sentry from #8 (error tracking only, defer analytics to v1.1)". This plan refines that into the sequence below.
...
2. **Account deletion ownership:** this plan flags account deletion as a v1 App Store blocker that's currently NOT in `bemy-v1-release-plan.md`. Do you want me to add it as a new item there (#10), or scope it inside this observability plan as part of the privacy work? Either way, 1 dev-day.
