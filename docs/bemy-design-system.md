...
**Voice:** warm, with surgical emoji use. The cake (🎂) appears on the birthday day only and exactly once. Every other state is a typographic treatment, not a graphical one. We are not a confetti app. (See `docs/bemy-pet-age-indicator-pm.md` for the full product positioning.)
...
- `docs/bemy-stitch-prompts.md` — design mockup prompts; left in place as a separate concern (not part of the design system).
