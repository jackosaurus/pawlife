...
> **Status:** Implemented as Bemy in May 2026. This doc is preserved as historical context — references to "Pawlife", placeholder names like `<NEW_NAME>`, and "the rename" reflect the state at the time of writing.
...
**Replaces:** ad-hoc threads referenced in `docs/bemy-v1-release-plan.md` item #3 ("rename to Boopa")
...
- **Was:** `https://jackdinh.github.io/bemy-legal/privacy.html`
- **Will be:** `https://jackdinh.github.io/<new-name>-legal/privacy.html`
- **Mechanism:** Rename the `bemy-legal` GitHub repo to `<new-name>-legal`. GitHub auto-redirects the old URL for ~12 months, but the in-app reference must point at the new canonical URL before the App Store Connect privacy URL field is updated.
...
Round 1 candidates (in `/Users/jackdinh/Code/pawlife/docs/bemy-rename-proposals.md`) and current standings:
...
**Round 2 candidates** (in `/Users/jackdinh/Code/pawlife/docs/bemy-rename-proposals-r2.md`):
...
**Steps:**
- [ ] Pick the final brand name. Read `/Users/jackdinh/Code/pawlife/docs/bemy-rename-proposals.md` and the round 2 doc if it exists. Lock `<NEW_NAME>` and `<new-name>`.
...
- [ ] Take baseline screenshots: EAS dashboard (Settings → slug + projectId + devices), Supabase dashboard label, current `bemy-legal` GitHub Pages site (`curl -I https://jackdinh.github.io/bemy-legal/privacy.html` should return `200 OK`).
...
- [ ] **Rename the `bemy-legal` GitHub repo first.** github.com → `jackosaurus/bemy-legal` → Settings → top of page "Repository name" → set to `<new-name>-legal` → Rename. GitHub auto-redirects the old URL for ~12 months. **Crucial timing trap:** the new URL `https://jackdinh.github.io/<new-name>-legal/privacy.html` must resolve `200 OK` BEFORE the code change in Phase 2 lands on `main`. Verify with `curl -I https://jackdinh.github.io/<new-name>-legal/privacy.html`. If GitHub Pages takes a few minutes to redeploy, wait.
- [ ] **Update the HTML body copy in the legal repo.** Edit `bemy-legal/index.html` and `bemy-legal/privacy.html` in the renamed repo (or in the local sibling directory `/Users/jackdinh/Code/pawlife/bemy-legal/` if you keep it as a subdirectory). Replace every `Pawlife` with `<NEW_NAME>` in titles and body. Commit + push. GitHub Pages redeploys in ~30–60 seconds. Re-verify with `curl`.
...
- [ ] **healthchecks.io check rename.** https://healthchecks.io → check `bemy-send-reminders` → pencil icon next to name → set to `<new-name>-send-reminders`. The ping UUID (the URL the Edge Function pings) is unchanged.
...
- [ ] Line 13: `'https://jackdinh.github.io/bemy-legal/privacy.html'` → `'https://jackdinh.github.io/<new-name>-legal/privacy.html'`
- [ ] Lines 2–8 comments mentioning "pawlife.app" and "bemy-legal" — update for accuracy.
...
- [ ] `bemy-legal/index.html` title + heading.
- [ ] `bemy-legal/privacy.html` title + body refs (lines 6, 57, 60, 62, 63, …). Use a sweep find-replace, then visually review — privacy policies are the kind of doc where a sloppy regex can corrupt sentences.

#### 2h. README + docs (low-risk; bulk find-replace acceptable)
- [ ] `README.md:1` — `# PawLife` → `# <NEW_NAME>`
- [ ] `docs/*.md` content references — bulk find-replace `Pawlife` → `<NEW_NAME>` and `pawlife` → `<new-name>` *(case-sensitive)*. ~78 lowercase + ~94 mixed-case references across 17 doc files. Verify the result for accidental corruption (especially in this very doc, file paths like `docs/bemy-rename-plan.md`, and references that intentionally mention the old name in historical context).
- [ ] **Defer doc filename renames to a follow-up commit AFTER this PR lands** — keeps git blame clean for the rename diff itself. Filenames like `bemy-rename-plan.md` can become `<new-name>-rename-plan.md` etc. in a follow-up.
...
**Rollback:** unlikely at this stage. If a critical issue surfaces (e.g. privacy URL 404 caught by a friend), revert the PR, redeploy the old `bemy-legal` site, rebuild under the old bundle ID. Don't delete the old `bemy-legal` repo for 90 days post-rename; it's the safety net.
...
| 4 (verification) | Critical issue caught on device | Revert Phase 2 merge; redeploy old `bemy-legal` if needed. Old GitHub repo still exists (don't delete for 90 days). Old PostHog/healthchecks names re-editable in UI. |

**Safety net:** do NOT delete the old `bemy-legal` GitHub repo for 90 days post-rename. GitHub Pages keeps serving it under the redirect, and it's the cleanest fallback for the privacy URL.
...
- `/Users/jackdinh/Code/pawlife/docs/bemy-rename-ops-audit.md` — external services audit (8 services, sequencing traps, time estimates).
- `/Users/jackdinh/Code/pawlife/docs/bemy-rename-proposals.md` — round 1 brand naming (Schnoot rejected).
- `/Users/jackdinh/Code/pawlife/docs/bemy-rename-proposals-r2.md` — round 2 brand candidates, cross-species and meme-energy. Top pick: Beastie. Includes disqualification notes for blep/sploot/critter/toebeans/bonk/borb.
- `/Users/jackdinh/Code/pawlife/docs/bemy-v1-release-plan.md` — item #3 ("Rename product to Boopa") explains the "poor life" problem motivating this rename.
...
- `/Users/jackdinh/Code/pawlife/bemy-legal/index.html` — title + heading.
- `/Users/jackdinh/Code/pawlife/bemy-legal/privacy.html` — title + lines 57, 60, 62, 63, etc.
...
`bemy-app-store-privacy-labels.md`, `bemy-build-plan.md`, `bemy-data-model.md`, `bemy-design-system.md`, `bemy-family-sharing.md`, `bemy-needs-attention-spec.md`, `bemy-pet-age-indicator-engineer.md`, `bemy-pet-age-indicator-pm.md`, `bemy-rename-ops-audit.md`, `bemy-rename-proposals.md`, `bemy-rename-plan.md` (this file), `bemy-roadmap.md`, `bemy-stitch-prompts.md`, `bemy-tech-stack.md`, `bemy-v1-account-deletion-code-review.md`, `bemy-v1-account-deletion-plan.md`, `bemy-v1-account-deletion-review.md`, `bemy-v1-migrations-013-014-db-review.md`, `bemy-v1-observability-plan.md`, `bemy-v1-posthog-plan.md`, `bemy-v1-posthog-review.md`, `bemy-v1-release-plan.md`.
...
| File | Line | Reference |
|---|---|---|
| `constants/legal.ts` | 13 | privacy URL contains `bemy-legal` |
...
|---|---|
| `bemy-legal/index.html` | title + heading |
| `bemy-legal/privacy.html` | title (line 6), heading (line 57), body refs (lines 60, 62, 63, …) |
