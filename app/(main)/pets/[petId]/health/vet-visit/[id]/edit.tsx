...
export default function EditVetVisitScreen() {
  ...
  const loadVetVisit = useCallback(async () => {
    try {
      ...
      setServerError('Failed to load vet visit');
    } finally {
      setLoading(false);
    }
  }, [id, reset]);
  ...
  const onSubmit = async (data: AddVetVisitFormData) => {
    ...
    try {
      await healthService.updateVetVisit(id, {
        date: data.date,
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to update vet visit';
